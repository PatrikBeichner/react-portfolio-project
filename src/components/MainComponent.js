...
class Main extends Component {
  ...
  render() {
    const TourWithId = ({ match }) => {
      return <TourInfo tour={this.state.tours.filter((tour) => tour.id === +match.params.tourId)[0]} />;
    };

    const RentalWithId = ({ match }) => {
      return <RentalInfo rental={this.state.rentals.filter((rental) => rental.id === +match.params.rentalId)[0]} />;
    };
